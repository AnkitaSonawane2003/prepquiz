...
//         const { data } = await API.get(`/api/tests/${testId}`);
//         const theTest = data.test || data;
//         if (!theTest || !Array.isArray(theTest.questions)) {
...
//   if (loading) return <div className="tt-loading">Loading test...</div>;
...
//                 ? q.options
//                 : Object.values(q.options || {}); // support if options is an object
//             return (
//               <li key={key}>
//                 <div className="q-text">
//                   <strong>{idx + 1}.</strong> {q.text}
...
//         const seconds = Number(theTest.duration) > 0 ? Number(theTest.duration) * 60 : 30 * 60;

//         const saved = localStorage.getItem(`testAttempt_${testId}`);
//         if (saved) {
//           const obj = JSON.parse(saved);
//           setSavedDraft({ answers: obj.answers || {}, timeLeft: obj.timeLeft ?? seconds });
//           setTimeLeft(obj.timeLeft ?? seconds);
//         } else {
//           setTimeLeft(seconds);
//         }
...
//     clearInterval(autosaveTimer.current);
//     autosaveTimer.current = setInterval(() => {
//       localStorage.setItem(`testAttempt_${testId}`, JSON.stringify({ answers, timeLeft }));
//     }, 10000);
//     return () => clearInterval(autosaveTimer.current);
//   }, [answers, timeLeft, started]);
...
//     setSubmitting(true);
//     try {
//       const token = localStorage.getItem("token") || localStorage.getItem("studentToken") || localStorage.getItem("jwt") || null;
//       if (!token) {
//         alert("Session expired. Please log in again.");
//         navigate("/studentlogin");
//         return;
...
//       const payload = { answers, timeTaken: (Number(test.duration) || 30) * 60 - (timeLeft ?? 0) };
//       const config = { headers: { Authorization: `Bearer ${token}` } };
//       const response = await API.post(`/api/testattempts/${testId}/submit`, payload, config);
...
//   if (loading) return <div className="tt-loading">Loading test...</div>;
//   if (pageError) return <div><h2>Error</h2><p>{pageError}</p><button onClick={() => navigate("/student/tests")}>Back</button></div>;
//   if (!test?.questions?.length) return <div><h2>{test.title}</h2><p>No questions found.</p><button onClick={() => navigate("/student/tests")}>Back</button></div>;

//   if (submittedAttempt) {
//     const score = submittedAttempt.totalObtained || submittedAttempt.score || 0;
//     const total = submittedAttempt.totalMarks || test.totalMarks || 0;
//     const percent = total ? ((score / total) * 100).toFixed(2) : null;
//     return (
//       <div className="test-take-container">
//         <header className="tt-header"><h1>{test.title} — Result</h1></header>
//         <div className="result-summary"><h2>Score: {score}/{total} {percent ? `(${percent}%)` : ""}</h2></div>
//         <button className="btn" onClick={() => navigate("/student/tests")}>Back</button>
//       </div>
//     );
//   }
...
//         <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
//           {!started && (
//             <button className="btn start-btn" onClick={() => {if (savedDraft) {const resume = window.confirm("Saved progress found. Resume it?"); startTest(resume);} else startTest(false);}}>▶ Start Test</button>
//           )}
//           <div className="tt-timer"><strong>Time left:</strong> {formatTime(timeLeft ?? 0)}</div>
//         </div>
//       </header>
...
//             const key = q._id || `q_${idx}`;
//             const ans = answers[key] || "";
//             const opts = Array.isArray(q.options) && q.options.length > 0 ? q.options : Object.values(q.options || {});
//             return (
//               <li key={key}>
//                 <div className="q-text"><strong>{idx + 1}.</strong> {q.topic ? `(${q.topic}) ` : ""}{q.text}</div>
//                 {q.type === "MCQ" && opts.length > 0 ? (
//                   <div className="options">
//                     {opts.map((opt, i) => {
//                       const letter = String.fromCharCode(65 + i);
//                       return (
//                         <label key={i} className="option-label">
//                           <input type="radio" name={`q_${key}`} disabled={!started || submitting} checked={ans === letter} onChange={() => selectOption(q._id, idx, letter)} />
...
//                   </div>
//                 ) : (
//                   <textarea rows={q.type === "Coding" ? 8 : 4} disabled={!started || submitting} value={ans} onChange={(e) => changeTextAnswer(q._id, idx, e.target.value)} placeholder="Write your answer..." />
//                 )}
//               </li>
//             );
//           })}
...
//         <div className="tt-actions">
//           <button type="submit" className="btn save-btn" disabled={!started || submitting}>{submitting ? "Submitting..." : "Submit Test"}</button>
//           <button type="button" className="btn" disabled={!started || submitting} onClick={() => {localStorage.setItem(`testAttempt_${testId}`, JSON.stringify({ answers, timeLeft })); alert("Progress saved locally!");}}>Save Progress</button>
//           <button type="button" className="btn" disabled={submitting} onClick={() => navigate("/student/tests")}>Quit</button>
//         </div>
//       </form>
//     </div>
//   );
...
//         const seconds = Number(theTest.duration) * 60 || 30 * 60;
//         const saved = localStorage.getItem(`testAttempt_${testId}`);
//         if (saved) {
//           const obj = JSON.parse(saved);
//           setSavedDraft({ answers: obj.answers || {}, timeLeft: obj.timeLeft ?? seconds });
...
//       const payload = {
//         answers,
//         timeTaken: (Number(test.duration) * 60 || 30 * 60) - (timeLeft ?? 0),
//       };
//       const config = { headers: { Authorization: `Bearer ${token}` } };
//       const response = await API.post(
//         `/api/testattempts/${testId}/submit`,
...
//   if (submittedAttempt) {
//     const score = submittedAttempt.totalObtained || 0;
//     const total = submittedAttempt.totalMarks || test.totalMarks || 0;
//     return (
//       <div className="test-take-container">
//         <h1>{test.title} — Result</h1>
...
//           onClick={() => {
//             if (savedDraft) {
//               const resume = window.confirm("Resume saved test?");
//               startTest(resume);
//             } else startTest(false);
//           }}
//         >
...
//         </button>
//       )}
//       <div>Time left: {formatTime(timeLeft ?? 0)}</div>
...
//             return (
//               <li key={key}>
//                 <div> {idx + 1} {q.text}  <strong>  (Marks :{q.marks} )</strong> (Topic : {  q.topic ? `${q.topic} ` : ""})
//                 </div>
//                 <div>
//                   {q.options.map((opt, i) => {
//                     const letter = String.fromCharCode(65 + i);
...
//         <button disabled={!started || submitting}>Submit Test</button>
//         <button
//           type="button"
//           disabled={!started || submitting}
//           onClick={() =>
